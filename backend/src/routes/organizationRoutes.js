...
const { sendOrganizationInviteEmail } = require('../utils/inviteEmailService');
...
router.get('/my-org', protect, async (req, res) => {
  try {
    ...
    const inviteLink = `${process.env.CLIENT_URL || 'http://localhost:5173'}/join/${organization.inviteCode}`;
...
router.post('/:id/invite-email', protect, isLeader, async (req, res) => {
  try {
    ...
    if (!organization) {
      return res.status(404).json({ message: 'Organization not found or you are not the leader' });
    }
...
// Delete organization (leader only)
router.delete('/:id', protect, isLeader, async (req, res) => {
  try {
    const organization = await Organization.findById(req.params.id);
    if (!organization) {
      return res.status(404).json({ message: 'Organization not found' });
    ...
    if (organization.leader.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: 'Only the organization leader can delete it' });
    }
    ...
    await JoinRequest.deleteMany({ organization: organization._id });
...
// Get organization by invite code (public route)
...
router.delete('/:orgId/members/:memberId', protect, isLeader, async (req, res) => {
  try {
    ...
    if (!organization.members.includes(memberToRemove)) {
      return res.status(400).json({ message: 'User is not a member of this organization' });
    }

    // Check if trying to remove the leader (not allowed)
    if (organization.leader.toString() === memberToRemove) {
      return res.status(400).json({ message: 'Cannot remove the organization leader' });
    }
